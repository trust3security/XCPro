        ...
        mapState.mapLibreMap?.let { map ->
            try {
                ...
                if (saved != null) {
                    ...
                    Log.d(TAG, "🎯 AAT: Restored camera position - lat=${saved.target?.latitude}, lon=${saved.target?.longitude}, zoom=${saved.zoom}")

                    // Clear saved position
                    savedCameraPosition = null
                } else {
                    Log.w(TAG, "🎯 AAT: No saved camera position to restore")
                }
            } catch (e: Exception) {
                Log.e(TAG, "🎯 AAT: Error restoring camera position: ${e.message}")
            }
        }
    }
}
