        ...
        try {
            ...
            val layer = SymbolLayer(LAYER_ID, SOURCE_ID)
                .withProperties(
                    ...
                    iconRotationAlignment("viewport"), // ✅ FIX: Rotate relative to screen, not map (prevents double rotation in TRACK_UP)
    ...
    fun updateLocation(
        ...
        try {
            ...
            if (!isValidCoordinate(location.latitude, location.longitude)) {
                Log.w(TAG, "❌ BLUE LOCATION: Invalid coordinates - lat:${location.latitude}, lon:${location.longitude}")
                return
            }
            ...
            Log.d(TAG, "✅ Location updated: lat=${location.latitude}, " +
                      "lon=${location.longitude}, track=${gpsTrack.toInt()}°, " +
                      "magHeading=${magneticHeading.toInt()}°, mode=$orientationMode, " +
                      "iconRotation=${iconRotation.toInt()}°")
