...
## 🎯 **CURRENT IMPLEMENTATION (2025-01-XX)**
...
## 🔧 **Critical Bug Fix: Ghost Dragging After FAB Exit**
...
fun CustomMapGestureHandler(
    ...
    LaunchedEffect(isAATEditMode) {
        if (!isAATEditMode) {
            Log.d(TAG, "🔧 RESET: External edit mode disabled - clearing local gesture state")
            aatEditModeIndex = -1
            isDraggingAAT = false
        }
    }
...
- **Intelligent Adaptive Zoom Calculation**:
  ...
  - **Formula**:
    ...
    metersPerPixel = diameterMeters / (minAvailableMapDimension × paddingFactor)
    zoom = log₂(earthCircumference / (tileSize × metersPerPixel)) + log₂(cos(latitude))
    ```
  - **Screen Layout Aware**:
...
- ❌ OLD BUG: Can still drag pin (ghost dragging)
...
1. **Double-tap detection**: `🎯 AAT: Double-tap detected on waypoint [X] - entering edit mode`
2. **Camera zoom in**: `🎯 AAT: Saved camera position - lat=..., lon=..., zoom=...`
3. **Screen size detection**: `🎯 AAT: Screen size - width=1080px, height=2340px, bottomSheet=500px, available=1080px`
4. **Adaptive padding calculation**: `🎯 AAT: Radius=10.0km → paddingFactor=50% → usablePixels=540px`
5. **Camera zoom calculation**: `🎯 AAT: Zoomed to turnpoint ${waypoint.title} (radius=10.0km) for edit mode`
6. **Dynamic zoom**: `🎯 AAT: Zooming to turnpoint - radius=10.0km, diameter=20.0km, screen=1080px, calculated zoom=12.04, at: ...`
7. **Edit mode entry**: `🎯 Entered AAT edit mode for waypoint [X]`
6. **Drag start**: `🎯 AAT: Started dragging pin [X]`
7. **FAB tap or double-tap outside**: `🎯 FAB: Exited AAT edit mode`
8. **Camera zoom out**: `🎯 AAT: Restored camera position - lat=..., lon=..., zoom=...`
9. **State reset**: `🔧 RESET: External edit mode disabled - clearing local gesture state`
...
- `MapCameraManager.kt`:
  ...
  - **Adaptive padding by circle size**:
    - ≤5km: 60% of screen (generous for small circles)
    - ≤10km: 50% of screen (balanced for medium circles)
    - ≤20km: 45% of screen (more padding for large circles)
  ...
  - Logarithmic zoom formula: `log₂(earthCircumference / (tileSize × metersPerPixel)) + log₂(cos(lat))`
