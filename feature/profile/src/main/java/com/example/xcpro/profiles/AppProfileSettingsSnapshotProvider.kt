...
class AppProfileSettingsSnapshotProvider @Inject constructor(
    ...
    override suspend fun buildSnapshot(
        ...
        val normalizedProfileIds = profileIds
            ...
            .toSortedSet()
        val requestedSectionIds = sectionIds.intersect(ProfileSettingsSectionSets.CAPTURED_SECTION_IDS)
        ...
        sections.putCapturedSection(requestedSectionIds, ProfileSettingsSectionIds.CARD_PREFERENCES) {
            captureCardPreferencesSection(normalizedProfileIds)
        }
        sections.putCapturedSection(
            requestedSectionIds,
        ...
        sections.putCapturedSection(requestedSectionIds, ProfileSettingsSectionIds.THEME_PREFERENCES) {
            captureThemeSection(normalizedProfileIds)
        }
        sections.putCapturedSection(requestedSectionIds, ProfileSettingsSectionIds.MAP_WIDGET_LAYOUT) {
        ...
        sections.putCapturedSection(requestedSectionIds, ProfileSettingsSectionIds.GLIDER_CONFIG) {
            captureGliderSection(normalizedProfileIds)
        }
        sections.putCapturedSection(requestedSectionIds, ProfileSettingsSectionIds.UNITS_PREFERENCES) {
        ...
        sections.putCapturedSection(
            ...
            captureMapStyleSection(normalizedProfileIds)
        }
        sections.putCapturedSection(
            requestedSectionIds,
            ProfileSettingsSectionIds.SNAIL_TRAIL_PREFERENCES
        ) {
            captureSnailTrailSection(normalizedProfileIds)
        }
        sections.putCapturedSection(
            requestedSectionIds,
        ...
        sections.putCapturedSection(requestedSectionIds, ProfileSettingsSectionIds.QNH_PREFERENCES) {
            captureQnhSection(normalizedProfileIds)
        }
        sections.putCapturedSection(
            requestedSectionIds,
    ...
    private fun captureMapStyleSection(profileIds: Set<String>): MapStyleSectionSnapshot {
        val stylesByProfile = profileIds.associateWith { profileId ->
            mapStyleRepository.readProfileStyle(profileId)
        }
        return MapStyleSectionSnapshot(stylesByProfile = stylesByProfile)
    }

    private fun captureSnailTrailSection(profileIds: Set<String>): SnailTrailSectionSnapshot {
        ...
        return SnailTrailSectionSnapshot(settingsByProfile = settingsByProfile)
    }
    ...
    private suspend fun captureQnhSection(profileIds: Set<String>): QnhSectionSnapshot {
        ...
        return QnhSectionSnapshot(valuesByProfile = valuesByProfile)
    }
    ...
    private suspend fun <T> MutableMap<String, JsonElement>.putCapturedSection(
        requestedSectionIds: Set<String>,
        sectionId: String,
        capture: suspend () -> T
    ) {
        if (!requestedSectionIds.contains(sectionId)) return
        this[sectionId] = gson.toJsonTree(capture())
    }
