...
object AATTargetOptimizer {
    ...
    private fun TaskPointDef.zoneRadius(): Double = when (val z = zone) {
        is com.trust3.xcpro.tasks.domain.model.SegmentOZ -> z.radiusMeters
        is com.trust3.xcpro.tasks.domain.model.CylinderOZ -> z.radiusMeters
        is com.trust3.xcpro.tasks.domain.model.SectorOZ -> z.radiusMeters
        is com.trust3.xcpro.tasks.domain.model.KeyholeOZ -> z.outerRadiusMeters
        is com.trust3.xcpro.tasks.domain.model.AnnularSectorOZ -> z.outerRadiusMeters
        is com.trust3.xcpro.tasks.domain.model.LineOZ -> z.widthMeters / 2.0
    }

    private fun TaskPointDef.zoneAngleRad(): Double = when (val z = zone) {
        is com.trust3.xcpro.tasks.domain.model.SegmentOZ -> Math.toRadians(z.angleDeg)
        is com.trust3.xcpro.tasks.domain.model.SectorOZ -> Math.toRadians(z.angleDeg)
        is com.trust3.xcpro.tasks.domain.model.KeyholeOZ -> Math.toRadians(z.angleDeg)
        is com.trust3.xcpro.tasks.domain.model.AnnularSectorOZ -> Math.toRadians(z.angleDeg)
        else -> 2 * PI
    }
