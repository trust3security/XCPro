...
class OgnOwnshipTargetBadgeRenderModelTest {
    ...
    @Test
    fun build_returnsNull_whenTargetIsOnScreen() {
        val model = OgnOwnshipTargetBadgeRenderModelBuilder.build(
            request(
                enabled = true,
                targetOnScreen = true,
    ...
    @Test
    fun build_formatsDistanceAndRelativeAltitude_withNavyForAboveOrLevel() {
        val model = OgnOwnshipTargetBadgeRenderModelBuilder.build(
            request(
                enabled = true,
                targetOnScreen = false,
                ...
                unitsPreferences = UnitsPreferences(distance = DistanceUnit.KILOMETERS)
            )
        )

        assertEquals("2.5 km\n+328 ft", model?.labelText)
    ...
    fun build_usesRedWhenTargetIsBelowOwnship() {
        ...
        assertEquals("1.2 km\n-100 m", model?.labelText)
        assertEquals(OgnOwnshipTargetBadgeRenderModelBuilder.BELOW_TEXT_COLOR_HEX, model?.textColorHex)
    }

    @Test
    fun build_usesFallbackMarkers_whenDistanceOrAltitudeAreUnknown() {
        val model = OgnOwnshipTargetBadgeRenderModelBuilder.build(
            request(
                enabled = true,
                targetOnScreen = false,
                target = target(distanceMeters = null, altitudeMeters = null),
                ownshipAltitudeMeters = 1_000.0,
                altitudeUnit = AltitudeUnit.METERS
            )
        )
    ...
    private fun target(distanceMeters: Double?, altitudeMeters: Double?): OgnTrafficTarget = OgnTrafficTarget(
        id = "T1",
        callsign = "CALL",
        destination = "APRS",
        latitude = -35.0,
