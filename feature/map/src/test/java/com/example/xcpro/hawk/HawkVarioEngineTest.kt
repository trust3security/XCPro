...
class HawkVarioEngineTest {
    ...
    private fun baroSample(altitudeM: Double, monoMs: Long): BaroData {
        val pressure = pressureForAltitude(altitudeM)
        return BaroData(
            pressureHPa = PressureHpa(pressure),
            timestamp = monoMs,
            monotonicTimestampMillis = monoMs
        )
    }

    private fun accelSample(value: Double, monoMs: Long): AccelData =
        AccelData(
