...
class HawkVarioEngine @Inject constructor() {
    ...
    fun updateAccel(sample: AccelData, config: HawkConfig) {
        ensureConfig(config)
    ...
    fun updateBaro(sample: BaroData, config: HawkConfig): HawkOutput? {
        ensureConfig(config)
        val monoMs = sample.monotonicTimestampMillis
    ...
    private fun pressureToAltitudeMeters(pressure: PressureHpa): Double {
        val pressureHpa = pressure.value
