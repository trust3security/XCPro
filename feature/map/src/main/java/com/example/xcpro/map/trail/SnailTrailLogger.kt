...
internal class SnailTrailLogger(
    ...
    fun logRenderEnd(renderId: Long, lineCount: Int, dotCount: Int) {
        if (!verboseEnabled) return
        Log.d(TAG_RENDER, "render#$renderId end lineFeatures=$lineCount dotFeatures=$dotCount")
    }
    ...
    fun logSegment(
        ...
        if (index >= maxSegmentLogs) {
            if (index == maxSegmentLogs) {
                Log.d(TAG_RENDER, "render#$renderId segment logs truncated at $maxSegmentLogs")
            }
            return
        }
