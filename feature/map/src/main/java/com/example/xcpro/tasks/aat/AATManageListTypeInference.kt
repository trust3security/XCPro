...
internal fun inferAATStartType(waypoint: TaskWaypoint): AATStartPointType =
...
internal fun inferAATFinishType(waypoint: TaskWaypoint): AATFinishPointType =
    when (waypoint.customPointType) {
        AATAreaShape.LINE.name -> AATFinishPointType.AAT_FINISH_LINE
        else -> AATFinishPointType.AAT_FINISH_CYLINDER
    }

internal fun inferAATTurnType(waypoint: TaskWaypoint, innerRadiusMeters: Double): AATTurnPointType =
    when {
