...
The executing agent (Codex) is responsible for implementing this plan from start to finish as a self-directed software engineering task.
...
- When ambiguity exists, make the most reasonable repo-consistent decision and document it in the commit message and/or PR notes.

## Responsibilities
- Implement all phases described in this document.
...
- Keep business logic pure and unit-testable (no Android framework calls in core logic).
- Use monotonic time for staleness/override and wall time only for UI timestamps (never mix sources).

## Workflow Rules
- Work phase-by-phase in order.
- Commit after each phase with a clear, scoped commit message.
- Do not leave TODOs or partial implementations in production code.
- If an existing test must change, justify it strictly as behavior parity.

## Definition of Done
Work is complete only when:
- All phases in this document are implemented.
- New unit tests cover the behavior checklist in this plan.
- `./gradlew testDebugUnitTest lintDebug assembleDebug` passes.
- The orchestrator/manager contains orchestration only (no business logic).
