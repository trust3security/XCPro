...
This section records what is already live or verified on the current Hetzner development server.

### Already done / verified

- Hetzner VM created and reachable over SSH
- Ubuntu updated and rebooted cleanly
- Docker stack running on the server
- PostgreSQL running for durable storage
- Redis running for hot live state
- FastAPI backend containerized with a real Dockerfile and image build
- Public API moved behind Caddy with automatic HTTPS
- DNS configured so `api.xcpro.com.au` points to the Hetzner server
- Backend port locked to localhost only (`127.0.0.1:8000`), so public traffic goes through Caddy/TLS
- `POST /api/v1/session/start` working over HTTPS
- share-code generation working
- write-token generation working
- follower read by share code working
- token-protected write endpoints working
- `POST /api/v1/position` working
- position validation and retry-safe dedupe working
- `POST /api/v1/task/upsert` working
- task revisioning working
- identical task upserts dedupe correctly
- session lifecycle implemented (`ACTIVE` / `STALE` / `ENDED` behavior)
- `POST /api/v1/session/end` working
- ended sessions reject further writes
- Alembic initialized and attached to the current database
- clean baseline migration created
- current Alembic revision verified as `5ae5bd538874 (head)`

### Not done yet / next hardening items

- move config/secrets into `.env`
- add dedicated `/health` and `/ready` endpoints if not already separated cleanly
- add Docker/container health checks
- add backup/restore scripts for PostgreSQL
- add rate limiting / abuse protection
- add observability cleanup (structured logs / metrics)
- formalize deployment scripts and host bootstrap steps
- implement future OGN / FLARM fusion work from the addendum

### Important scope note

The current server is a **working development backend foundation**, not a finished production competition server.
It already supports the core relay-first live-follow flow, but it does **not** yet implement full OGN/FLARM identity fusion, scoring, dashboards, or broader production hardening.
...
### Explicit “do not infer same-task from OGN alone” rule
