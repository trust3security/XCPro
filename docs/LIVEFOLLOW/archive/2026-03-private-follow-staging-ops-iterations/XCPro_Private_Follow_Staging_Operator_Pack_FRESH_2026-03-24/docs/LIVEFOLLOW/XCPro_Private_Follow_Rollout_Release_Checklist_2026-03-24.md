...
- `python scripts/private_follow_env_preflight.py` passes in the target server runtime
...
- `alembic upgrade head` is already proven to work on a fresh empty DB in repo
...
Use `XCPro_Private_Follow_Staging_Smoke_Guide_2026-03-24.md` as the operational
execution owner so results are recorded against the current implemented flow
instead of freehand notes.
