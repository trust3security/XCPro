...
## 1.7 Task Architecture Compliance Refactor Guidance (pre-filled for this task)

Scope:
- Refactor task management (AAT and Racing) to comply with `docs/ARCHITECTURE/ARCHITECTURE.md`
  and `docs/ARCHITECTURE/CODING_RULES.md`.
- Remove Android/UI dependencies from task domain logic.
- Keep behavior parity unless explicitly stated.

Current issues to resolve:
- Task managers depend on Context, MapLibre, and Compose runtime state.
- Task state is duplicated between legacy managers and `TaskRepository`.
- Task map rendering and edit interactions are embedded in non-UI layers.
- Persistence is embedded in managers/coordinator rather than repositories.

Target architecture shape:
- Domain engines (`AATTaskEngine`, `RacingTaskEngine`) with `StateFlow` SSOT.
- Persistence via injected repositories/interfaces (no SharedPreferences in domain).
- UI-only renderers/controllers for MapLibre tasks.
- Coordinator becomes a pure router (no MapLibre or Context).

Authoritative plan:
- See `docs/refactor/Task_Architecture_Compliance_Refactor_Plan.md`.
...
## 2.5 Task Architecture Compliance Refactor Plan (expanded)

Phase 0 (Baseline):
- Inventory task call sites and current behavior.
- Add regression tests for add/remove/reorder, task type switch, AAT targets, racing distance.

Phase 1 (Domain contracts):
- Define pure task engine interfaces and task state models (no UI types).

Phase 2 (Pure engines):
- Implement `AATTaskEngine` and `RacingTaskEngine` using existing modules.
- Replace `mutableStateOf` with `StateFlow`.

Phase 3 (Data layer extraction):
- Introduce persistence interfaces and adapters for AAT/Racing.
- Add task type settings repository.

Phase 4 (UI rendering):
- Move MapLibre rendering/editing to UI-only renderers/controllers.

Phase 5 (Coordinator refactor):
- Convert coordinator to a pure router (no MapLibre, no Context).

Phase 6 (UI wiring):
- Update ViewModels and overlays to read from engine flows.

Phase 7 (Navigation + replay):
- Update navigation/replay integrations to use engine flows.

Phase 8 (Cleanup + enforcement):
- Remove legacy UI APIs from domain.
...
## 3.4 Task Architecture Compliance Refactor Acceptance (pre-filled)
- [ ] Task domain code has no Android, MapLibre, or Compose runtime dependencies.
- [ ] Single SSOT for task state via engine/repository flow.
- [ ] Map rendering/editing is UI-only and uses task state inputs.
- [ ] Task coordinator is a pure router (no UI types).
- [ ] AAT and Racing remain fully isolated (no cross-imports).
- [ ] Tests cover task mutations, task type switching, and distance calculations.
...
Decision:
- Task domain logic extracted into pure engines; UI rendering moved to UI-only renderers.
Alternatives considered:
- Keep MapLibre rendering inside task managers.
- Keep dual SSOT (legacy manager + repository).
Why chosen:
- Aligns with architecture rules for SSOT and dependency direction.
Impact / risks:
- Requires careful migration to avoid behavior drift.
Follow-ups:
- Ensure parity tests cover all task mutations and navigation.
