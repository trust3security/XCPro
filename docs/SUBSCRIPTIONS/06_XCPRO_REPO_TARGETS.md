...
- `app/src/main/java/.../MainActivityScreen.kt`
- `app/src/main/java/.../AppNavGraph.kt`
- existing auth/session entry point or root auth gate if present

### Likely feature / settings entry points

- existing sign-in / profile / settings area
...
- `core/billing/...`
- existing auth slice only as needed for root account-required wiring
...
### `MainActivityScreen`
- resolve sign-in state first
- if signed out, route to sign-in / create-account
- if signed in, collect root entitlement state
...
### `AppNavGraph`
- route to sign-in when auth is missing
...
Do not put the whole subscription system directly into `app` just because it is quicker.
Create a dedicated billing core now if the feature is intended to survive.

## Explicit repo cautions
...
- Do not invent guest mode to bypass unfinished auth work.
- If any old path assumptions in non-Markdown templates conflict with this file, this file wins.
