...
4) Temporary hotspot line-budget exception pending focused ownership split
- Rule:
...
- Risk:
  - This file temporarily bypasses its stricter hotspot cap, so unreviewed growth could hide renewed ownership drift in task orchestration seams.
- Rationale:
  - The file remains below the repo-wide default `<= 500` cap, but currently exceeds a narrower hotspot budget by a small margin.
  ...
  - A time-boxed exception is safer than silently loosening the hotspot policy for the remaining task hotspot.
- Mitigation:
  ...
  - Do not add more logic to this file without first extracting focused helpers or bindings.
- Removal steps:
  - Reduce the scoped file back to its configured hotspot cap through focused ownership splits.
