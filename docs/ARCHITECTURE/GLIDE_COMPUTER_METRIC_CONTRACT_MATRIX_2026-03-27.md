...
| current L/D | recent measured over-ground glide ratio from runtime flight metrics, not final glide and not polar best L/D | IMPLEMENTED | `FlightDataRepository` -> adapter | Explicit valid/invalid only; no degraded state in the active contract |
| L/D vario | measured through-air glide ratio from authoritative non-GPS TAS and TE vario in straight flight only | IMPLEMENTED | flight runtime -> adapter | Valid only when flying, TAS is authoritative, TE is available, sink is credible, and the aircraft is neither circling nor turning |
