...
- Positive → rising airmass (street / convergence / weak lift)
- Negative → sinking airmass
...
### 3.4 TAS → IAS conversion (for polar lookup)

```
IAS ≈ TAS × sqrt(rho / rho0)
```
...
Advanced setting:
- **ISA temperature offset**: −10 °C … +10 °C (default 0)
...
```
w_meas = LPF(d(alt_baro)/dt, tau ≈ 2.0 s)
```
...
- 20+ circles ⇒ high confidence

### 6.2 Confidence decay after leaving a thermal
Wind confidence must decay with **time or distance**:

Recommended:
- Half confidence every **5–10 minutes**, or
- Half confidence every **10–20 km** since last circling solve
...
glideNetto_raw = w_meas − sink_expected
```

Because `sink_expected` is negative, subtracting it adds the glider’s expected sink back.
...
- 80-kt glider → longer tau
- 120-kt glider → shorter tau

But both average the **same length of sky**.

### 9.3 Optional second output
- **Trend Glide-Netto**: fixed long window (≈1.5 km)
...
- A **trend netto** is consistently negative:
  ...
  - sustained for `>= 3–5 minutes` (default 4 minutes)
...
Use a long-window glide-netto (trend) suitable for final glide. If only one glide-netto is implemented, compute an internal trend via a long time window (>= 3–5 minutes) and do not display it as a twitchy needle.

#### 9.5.2 “Arrival margin shrinking faster than expected”
Define:
- `arrival_margin_m` = predicted arrival height above required (meters)
- `arrival_margin_rate_mps` = d(arrival_margin_m)/dt (m/s), low-passed (tau 30–60s)
...
```
MC ≈ expected average climb in next thermals
```
...
MC_auto = rate_limited(MC_auto → MC_raw)
MC_auto = clamp(MC_min, MC_max)
```

Recommended limits:
- Max change rate: 0.2–0.4 m/s per minute
- Typical MC range: 0–4 m/s (configurable)
...
```
MC_effective = clamp(MC_auto − glideNetto, 0, MC_max)
```

- Rising air → slightly slower
- Sink → slightly faster
...
- Optional: show **speed delta** (±kt) instead of raw MC math
...
- Low wind confidence → freeze or heavily smooth glide-netto
- No recent circling → degrade confidence over time
- Sensor spikes → gated out, never fed into MC or glide-netto
...
- Still air → glide-netto ≈ 0
- Known sink band → glide-netto negative
- Weak lift line → glide-netto positive
- 80 kt vs 120 kt → same behaviour for same distance window
- Thermal exit → Auto-MC updates once, smoothly
