...
# 8) Next-Run Continuation Block (Use After Main Contract Completion)

Use this section to define what should happen immediately after a successful
P0/P1 delivery or after the first execution pass.

## 8.1 Immediate Next Steps (Default Sequence)
1. Run full instrumentation verification:
   - `./gradlew --no-configuration-cache :app:connectedDebugAndroidTest --no-parallel "-Pandroid.injected.androidTest.leaveApksInstalledAfterRun=true"`
   - `./gradlew --no-configuration-cache connectedDebugAndroidTest --no-parallel`
2. Execute P2 cleanup:
   - Remove transitional compatibility wrappers that are no longer needed in internal paths.
   - Tighten static SI/architecture guards to block drift (for example ad-hoc conversion patterns in domain/task/replay internals).
3. Execute compliance closeout:
   - Perform one more deep re-pass.
   - Update findings/backlog/plan docs with final status.
   - Record any intentional deferrals with owner and rationale.
...
- Pass/fail outcome.
- Fixes made (if any).
- Remaining risk and explicit follow-up owner.

## 8.3 Autonomous Multi-Run Runbook (Wrapper-Cut Then Polar Migration)
Use this when deferred work includes compatibility-wrapper removal plus polar SI migration.

Mandatory order:
1. Run A: compatibility-wrapper cut in internal task/AAT/racing paths.
2. Run B: polar/config storage migration to SI canonical (`m/s`) with compatibility read for legacy `km/h`.
3. Run C: residual SI contract normalization (radius contracts, delegate/calculator wrappers, interactive distance contracts, scoring and validation speed contracts).
4. Run D: compliance closeout (adapter tests, deep re-pass, docs sync, stale/dead contract cleanup, final verification).

Locked decisions:
1. Internal canonical units remain SI only.
2. Polar canonical persisted/internal contract is `m/s`.
3. `km/h` is boundary-only (UI/input/import/export/legacy-read compatibility).
4. Compatibility wrappers are boundary-only after Run A.

Run A acceptance gate:
1. No internal manager/coordinator/use-case/viewmodel logic depends on `*Km` wrappers.
2. Remaining wrappers are boundary-only and explicitly named/documented.
3. Required verification commands in Section 4 pass.

Run B acceptance gate:
1. Polar internal math/storage paths are SI-only.
2. Legacy `km/h` compatibility read path is tested.
3. Required verification commands in Section 4 pass.

Run C acceptance gate:
1. Remaining internal distance/speed contracts are SI-only (`meters`, `m/s`) with explicit boundary formatting.
2. Internal `*Km` wrappers in coordinator/use-case/viewmodel/manager/delegate/calculator surfaces are removed or boundary-justified.
3. Required verification commands in Section 4 pass.

Run D acceptance gate:
1. Backlog/docs updated with final status.
2. Deep re-pass reports no new SI drift in scoped modules.
3. Full verification matrix (Section 4) completed or limitations recorded per Section 4.2.

Autonomous stop conditions:
1. A migration decision cannot be inferred from repository contracts/tests and would alter persisted-data semantics.
2. Missing/inconsistent files prevent safe contract migration.
3. Verification failures are unrelated/pre-existing and block attribution.

If blocked:
1. Apply only safe non-breaking changes.
2. Document blocker, impact, and proposed decision.
3. Mark affected backlog item as blocked with owner and follow-up step.
